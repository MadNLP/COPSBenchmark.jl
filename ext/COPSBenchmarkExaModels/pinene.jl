...
function COPSBenchmark.pinene_core()
    ...
    core, theta = add_var(core, 1:np; lvar = zero_T, start = zero_T)
    core, v = add_var(core, 1:args.nh, 1:ne;
        start = Deferred(a -> [_v0(a)[i, s] for i = 1:a.nh, s = 1:ne]))
    core, w = add_var(core, 1:args.nh, 1:nc, 1:ne; start = zero_T)
    core, uc = add_var(core, 1:args.nh, 1:nc, 1:ne;
        start = Deferred(a -> begin v0 = _v0(a); [v0[i,s] for i = 1:a.nh, j = 1:nc, s = 1:ne] end))
    core, Duc = add_var(core, 1:args.nh, 1:nc, 1:ne; start = zero_T)

    itr = Deferred(a -> begin
        h = tf / a.nh
        t = [(i-1)*h for i in 1:a.nh+1]
        itau = Int[min(a.nh, Int(floor(tau[i]/h))+1) for i in 1:nm]
        [(j, s, itau[j], tau[j], t[itau[j]], z[j,s]) for j = 1:nm, s in 1:ne]
    end)
    # l2 error
    core, _ = add_obj(core,
        (v[it,s] + sum(w[it,k,s]*(tj-ti)^k/(factorial(k)*h^(k-1)) for k in 1:nc) - zjs)^2
        for (j, s, it, tj, ti, zjs) in itr)

    # Collocation model
    itr2 = [(j, rho[j]) for j = 1:nc]
    core, _ = add_con(core,
        - uc[i, j, s] + v[i,s] + h*sum(w[i,k,s]*(rhoj^k/factorial(k)) for k in 1:nc)
        for i = 1:args.nh, (j, rhoj) in itr2, s = 1:ne)
    core, _ = add_con(core,
        - Duc[i, j, s] + sum(w[i,k,s]*(rhoj^(k-1)/factorial(k-1)) for k in 1:nc)
        for i = 1:args.nh, (j, rhoj) in itr2, s = 1:ne)
    # Boundary
    core, _ = add_con(core, -v[1, s] + bcs for (s, bcs) in enumerate(bc))
    # Continuity
    core, _ = add_con(core,
        v[i, s] + sum(w[i, j, s]*h/factorial(j) for j in 1:nc) - v[i+1, s]
        for i = 1:args.nh-1, s = 1:ne)
    core, _ = add_con(core, -Duc[i,j,1] - (theta[1]+theta[2])*uc[i,j,1] for i = 1:args.nh, j = 1:nc)
    core, _ = add_con(core, -Duc[i,j,2] + theta[1]*uc[i,j,1] for i = 1:args.nh, j = 1:nc)
    core, _ = add_con(core, -Duc[i,j,3] + theta[2]*uc[i,j,1] - (theta[3]+theta[4])*uc[i,j,3] + theta[5]*uc[i,j,5] for i = 1:args.nh, j = 1:nc)
    core, _ = add_con(core, -Duc[i,j,4] + theta[3]*uc[i,j,3] for i = 1:args.nh, j = 1:nc)
    core, _ = add_con(core, -Duc[i,j,5] + theta[4]*uc[i,j,3] - theta[5]*uc[i,j,5] for i = 1:args.nh, j = 1:nc)
    core
end
