...
function COPSBenchmark.robot_model(::ExaModelsBackend, nh; T = Float64, backend = nothing, kwargs...)
    ...
    ExaModels.@con(
        core, c11, - phi[1] + pi / 4.0
    )
    ExaModels.@con(
        core, c12, - rho[nh+1] + 4.5
    )
    ExaModels.@con(
        core, c13, - the[nh+1] + 2.0 * pi / 3
    )
    ExaModels.@con(
        core, c14, - phi[nh+1] + pi / 4.0
    )
    ExaModels.@con(
        core, c15, - rho_dot[1] + 0.0
    )
    ExaModels.@con(
        core, c16, - the_dot[1] + 0.0
    )
    ExaModels.@con(
        core, c17, - phi_dot[1] + 0.0
    )
    ExaModels.@con(
        core, c18, - rho_dot[nh+1] + 0.0
    )
    ExaModels.@con(
        core, c19, - the_dot[nh+1] + 0.0
    )
    ExaModels.@con(
        core, c20, - phi_dot[nh+1] + 0.0
    )
