...
@inline function COPSBenchmark.glider_recipe(
    ...
    # These were `@add_expr`s.  That macro stores its body as a closure, which a
    # recipe cannot instantiate; a local helper is spliced at each point of use
    # exactly as the subexpression was, so the trees are unchanged.  They chain,
    # as the originals did -- u is written in terms of r, v in terms of w.
    r(i) = (x[i]/r_0 - r_offset)^2
    u(i) = u_c*(one_T - r(i))*exp(-r(i))
    w(i) = vy[i] - u(i)
    v(i) = sqrt(vx[i]^2 + w(i)^2)
    D(i) = half*(cd0+cd1*cL[i]^2)*rho*S*v(i)^2
    L(i) = half*cL[i]*rho*S*v(i)^2
    vx_dot(i) = (-L(i)*(w(i)/v(i)) - D(i)*(vx[i]/v(i)))/m
    vy_dot(i) = (L(i)*(vx[i]/v(i)) - D(i)*(w(i)/v(i)))/m - g
...
@inline function COPSBenchmark.glider_args(::ExaModelsBackend, nh; T = Float64)
