...
# 1:1 transcription of _tetra_model_exa (tetra.jl) returning the core.
function _lazy_tetra(x0, TETS, Const)
    τ = 0.0
    n = length(x0)
    N = round(Int, n / 3)
    E = round(Int, length(TETS) / 4)
...
COPSBenchmark.tetra_duct12_core() = _lazy_tetra(xe_duct12, TETS_duct12, Const_duct12)
COPSBenchmark.tetra_duct15_core() = _lazy_tetra(xe_duct15, TETS_duct15, Const_duct15)
COPSBenchmark.tetra_duct20_core() = _lazy_tetra(xe_duct20, TETS_duct20, Const_duct20)
COPSBenchmark.tetra_hook_core() = _lazy_tetra(xe_hook, TETS_hook, Const_hook)
COPSBenchmark.tetra_foam5_core() = _lazy_tetra(xe_foam5, TETS_foam5, Const_foam5)
COPSBenchmark.tetra_gear_core() = _lazy_tetra(xe_gear, TETS_gear, Const_gear)
