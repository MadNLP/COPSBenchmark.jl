...
@inline function COPSBenchmark.catmix_model(::ExaModelsBackend, nh; T = Float64, backend = nothing, kwargs...)
    ...
    ExaModels.@var(c, u, nh, nc; lvar = zeros(nh, nc), uvar = ones(nh, nc), start = zeros(nh, nc))
    ExaModels.@var(c, v, nh, ne; start = [mod(j, ne) for i in 1:nh, j in 1:ne])
    ExaModels.@var(c, w, nh, nc, ne; start = zeros(nh, nc, ne))
    ExaModels.@var(c, pp, nh, nc, ne; start = [mod(k, ne) for i in 1:nh, j in 1:nc, k in 1:ne])
    ExaModels.@var(c, Dpp, nh, nc, ne; start = zeros(nh, nc, ne))
    ExaModels.@var(c, ppf, ne; start = [mod(i,ne) for i in 1:ne])

    ExaModels.@obj(c, -1.0 + ppf[1] + ppf[2])
    ExaModels.@obj(c, alpha/h*(u[i+1, j] - u[i, j])^2 for i in 1:nh-1, j in 1:nc)
