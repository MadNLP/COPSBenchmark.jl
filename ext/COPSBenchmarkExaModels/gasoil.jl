...
@inline function COPSBenchmark.gasoil_model(::ExaModelsBackend, nh; T = Float64, backend = nothing, kwargs...)
    ...
    ExaModels.@add_con(core, c3, - v[1, s] + z1s for (s,z1s) in itr3) #TODO
    # Continuity
    ExaModels.@add_con(
        core,
        c4,
...
function COPSBenchmark.gasoil_core()
    ...
    # L2 error
    core, _ = add_obj(core,
        (v[itauj,s] + sum(w[itauj,k,s]*(tauj-tj)^k/(factorial(k)*h^(k-1)) for k in 1:nc) - zjs)^2
        for (j, s, itauj, tauj, tj, zjs) in itr)
    ...
    # Boundary
    itr3 = [(s, z[1, s]) for s = 1:ne]
    core, _ = add_con(core, - v[1, s] + z1s for (s, z1s) in itr3)
    # Continuity
    core, _ = add_con(core,
