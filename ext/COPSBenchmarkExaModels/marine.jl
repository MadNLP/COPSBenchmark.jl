...
function COPSBenchmark.marine_core()
    ...
    rho = [0.5]
    tau = collect(Float64, range(0.0, 10.0, 21))
    tf = tau[nm]
    h = tf / args.nh
    zero_T = 0.0
    ...
    # Growth rates
    core, g = add_var(core, 1:ne-1; lvar = zero_T)
    # Mortality rates
    core, m = add_var(core, 1:ne; lvar = zero_T)
    ...
    core, _ = add_con(core,
        - Duc[i,j,s] + g[s-1]*uc[i,j,s-1] - (m[s]+g[s])*uc[i,j,s]
        for i = 1:args.nh, j = 1:nc, s = 2:ne-1)
    core
end
