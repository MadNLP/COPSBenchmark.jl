...
function pinene_model(nh)
    ...
    @constraints(
        model, begin
            [i=1:nh, j=1:nc], Duc[i,j,1] == - (theta[1]+theta[2])*uc[i,j,1]
            [i=1:nh, j=1:nc], Duc[i,j,2] == theta[1]*uc[i,j,1]
            [i=1:nh, j=1:nc], Duc[i,j,3] == theta[2]*uc[i,j,1] - (theta[3]+theta[4])*uc[i,j,3] + theta[5]*uc[i,j,5]
            [i=1:nh, j=1:nc], Duc[i,j,4] == theta[3]*uc[i,j,3]
            [i=1:nh, j=1:nc], Duc[i,j,5] == theta[4]*uc[i,j,3] - theta[5]*uc[i,j,5]
        end
    )
    return model
end
